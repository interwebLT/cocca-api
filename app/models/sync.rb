module Sync
  ...
  def self.sync records
    records.each do |master|
      ...
      master.domains.each do |domain|
        ...
        if domain.transfer_domain?
          TransferDomainJob.perform_later domain.partner, domain.as_json
          RenewDomainJob.perform_later    domain.partner, domain.as_json if domain.transfer_with_renew?
        end
      end
      ...
      master.hosts.each do |host|
        ...
        host.host_addresses.each do |host_address|
          ...
          DeleteHostAddressJob.perform_later host.partner, host_address.as_json if host_address.delete_operation?
        end
      end
    end
    ...
    records.each do |master|
      master.domains.each do |domain|
        UpdateDomainJob.perform_later domain.partner, domain.as_json if domain.update_domain? and !domain.pending_transfer?
